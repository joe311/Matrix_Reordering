...
def SI_sort(M, mode='recurrency', jac=1, response='silent', pauli_fac=1.0, return_object='single'):
    ...
    if jac == 1:
        ...
        if mode == 'recurrency':
            res = minimize(calc_recurrency_cost, z, jac=calc_recurrency_jacobian, method=method, tol=rec_tol,
                           bounds=z_bounds, options=options)

        if mode == 'bandwidth':
            res = minimize(calc_bandwidth_cost, z, jac=calc_bandwidth_jacobian, method=method, tol=bwt_tol,
                           bounds=z_bounds, options=options)
